  ...
  override fun configureFlutterEngine(@NonNull engine: FlutterEngine) {
    ...
    MethodChannel(engine.dartExecutor.binaryMessenger, CHANNEL).setMethodCallHandler { call, result ->
      when (call.method) {
        ...
        // START VPN (flows through prepareVpn)
        "startVpn" -> {
          ...
          pendingPerAppMode = call.argument<String>("perAppMode") // optional

           // NEW: read useLocalBlocklist flag (default false)
        ...
        // List installed apps (returns list of maps)
        ...
        // Return base64 PNG icon for package (empty string on failure)
        "getAppIcon" -> {
         ...
         * - operation: "add"|"remove"|"set"|"clear"  (default: "add")
         *
         * Behavior:
         * - maintains two prefs sets: "allowed_apps" and "blocked_apps"
         * - when adding to allowed, remove those apps from blocked; when adding to blocked, remove them from allowed
         */
        "updatePerAppFilter" -> {
          try {
            ...
            // Start with extras so DnsVpnService can pick up immediately
            Intent(this, DnsVpnService::class.java).apply {
              action = DnsVpnService.ACTION_START
              // we pass allowed list as extra to allow immediate apply
        ...
        // updateBlocklist (domain list) - stored separately
        "updateBlocklist" -> {
          try {
            val domains = call.argument<List<String>>("domains") ?: emptyList()
            val prefs = getSharedPreferences("shieldguard_prefs", MODE_PRIVATE)
            prefs.edit().putStringSet("blocklist_domains", domains.toSet()).apply()
            result.success(true)
          } catch (e: Exception) {
        ...
        // Return current saved per-app prefs (allowed + blocked)
        "getPerAppPrefs" -> {
          try {
            val prefs = getSharedPreferences("shieldguard_prefs", MODE_PRIVATE)
            ...
            if (oldMode != null && oldPackages != null) {
              // migrate into allowed/blocked sets (only once)
              val allowed = prefs.getStringSet("allowed_apps", emptySet())?.toMutableSet() ?: mutableSetOf()
              val blocked = prefs.getStringSet("blocked_apps", emptySet())?.toMutableSet() ?: mutableSetOf()
              if (oldMode == "allow") {
        ...
        // Admin / Device owner helpers (unchanged)
        "isDeviceOwner" -> {
          try {
    ...
    if (requestCode == VPN_REQUEST && resultCode == Activity.RESULT_OK) {
      val svcIntent = Intent(this, DnsVpnService::class.java).apply {
        ...
        pendingPerAppMode?.let { putExtra("perAppMode", it) }
           // NEW: pass useLocalBlocklist into the service
